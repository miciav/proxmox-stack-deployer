...
class TestMainFunction(unittest.TestCase):
    ...
    def test_main_destroy_workflow(self, mock_parse_args, mock_exit, mock_os_system, 
                                  mock_terraform_destroy, mock_ansible_destroy):
    ...
    @patch('deploy.parse_arguments')
    def test_main_full_deployment(self, mock_parse_args, mock_docker_install, 
                                 mock_k3s_install, mock_vm_config, mock_nat_config,
                                 mock_terraform_deploy, mock_initial_setup):
    ...
    @patch('deploy.run_ansible_docker_installation')
    @patch('deploy.parse_arguments')
    def test_main_skip_ansible(self, mock_parse_args, mock_docker_install, 
                              mock_k3s_install, mock_vm_config, mock_nat_config,
                              mock_terraform_deploy, mock_initial_setup):
    ...
    @patch('deploy.run_ansible_docker_installation')
    @patch('deploy.parse_arguments')
    def test_main_selective_skips(self, mock_parse_args, mock_docker_install, 
                                 mock_k3s_install, mock_vm_config, mock_nat_config,
                                 mock_terraform_deploy, mock_initial_setup):
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_destroy(self, mock_exit, mock_os_system, mock_subprocess):
        ...
        expected_calls = [
            call("ansible-playbook -i ./inventories/inventory-nat-rules.ini ./playbooks/remove_nat_rules.yml", shell=True, check=True),
